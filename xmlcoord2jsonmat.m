...
for iR = 1:length(data)
   ...
   json_struct.slice(iR).transf_mat = [json_struct.slice(iR).ux json_struct.slice(iR).vx json_struct.slice(iR).ox;...
       json_struct.slice(iR).uy json_struct.slice(iR).vy json_struct.slice(iR).oy;...
       json_struct.slice(iR).uz json_struct.slice(iR).vz json_struct.slice(iR).oz];
end
